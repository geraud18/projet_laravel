...
trait SingleStepPaymentTrait
{
	...
	protected function paymentSettings()
	{
		...
		// Messages
		if ($isNewEntry) {
			$this->apiMsg['post']['success'] = t('your_listing_has_been_created');
		} else {
			$this->apiMsg['post']['success'] = t('your_listing_has_been_updated');
		}
		...
		if ($isNewEntry) {
			// Share the Post's latest payment info variables without passing Listing as argument
